  #include <math.h>
    ...
    Estr *buscaEstr(Estr *i, char estr[]){
        Estr *lista;
        for(lista = i; lista != NULL; lista = lista->prox) {
            if(strcmp(lista->nome, estr) == 0){
                return lista;
            }
        }
        return NULL;
...
double eval(Ast *a) {
	...
	switch(a->nodetype) {
		...
		case 'N': v = buscaEstr(list, ((Varval *)a)->var)->valor; break;	
		...
		case '=':
			v = eval(((Symasgn *)a)->v);
			Estr *aux = buscaEstr(list, ((Varval *)a)->var);	
			aux->valor = v;			
			break;
